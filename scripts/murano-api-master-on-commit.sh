...
cd murano-tests ; git fetch https://tnurlygayanov@review.openstack.org/stackforge/murano-tests refs/changes/63/73263/2 && git checkout FETCH_HEAD ; cd ..
expect murano-tests/infra/deploy_component.sh ubuntu 10.100.0.55 $GERRIT_REFSPEC murano-api
rm -rf murano-tests
...
sed -i "s/murano_url = http:\/\/127.0.0.1:8082/murano_url = http:\/\/10.100.0.55:8082/" etc/tempest.conf
sed -i "s/murano_metadata = http:\/\/127.0.0.1:8084/murano_metadata = http:\/\/10.100.0.55:8084/" etc/tempest.conf
